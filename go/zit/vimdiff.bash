...
debug_print() {
  ...
  if test -n "$GIT_MERGETOOL_VIMDIFF_DEBUG"; then
    >&2 echo "$@"
  fi
}
...
gen_cmd_aux() {
  ...
  oldIFS=$IFS
  IFS=#
  for c in $(echo "$LAYOUT" | sed 's:.:&#:g'); do
    if test -z "$c" || test "$c" = " "; then
      continue
    fi
    ...
    if test "$nested" -lt "$nested_min"; then
      nested_min=$nested
    fi
  done
  IFS=$oldIFS
  ...
  while test "$nested_min" -gt "0"; do
    start=$((start + 1))
    end=$((end - 1))

    start_minus_one=$((start - 1))

    while ! test "$(substring "$LAYOUT" "$start_minus_one" 1)" = "("; do
      start=$((start + 1))
      start_minus_one=$((start_minus_one + 1))
    done

    while ! test "$(substring "$LAYOUT" "$end" 1)" = ")"; do
      end=$((end - 1))
    done
  ...
  oldIFS=$IFS
  IFS=#
  for c in $(substring "$LAYOUT" "$start" "$((end - start))" | sed 's:.:&#:g'); do
    i=$((i + 1))
    ...
    if test "$nested" = 0; then
      current=$c

      if test "$current" = "/"; then
        if test -z "$index_horizontal_split"; then
          index_horizontal_split=$i
        fi

      elif test "$current" = ","; then
        if test -z "$index_vertical_split"; then
  ...
  elif ! test -z "$index_vertical_split"; then
  ...
  if test "$target" = "LOCAL"; then
    CMD="$CMD | 1b"

  elif test "$target" = "BASE"; then
    CMD="$CMD | 2b"

  elif test "$target" = "REMOTE"; then
    CMD="$CMD | 3b"

  elif test "$target" = "MERGED"; then
    CMD="$CMD | 4b"
...
gen_cmd() {
  ...
  if echo "$LAYOUT" | grep @LOCAL >/dev/null; then
    FINAL_TARGET="LOCAL"
  elif echo "$LAYOUT" | grep @BASE >/dev/null; then
    FINAL_TARGET="BASE"
  else
    FINAL_TARGET="MERGED"
  fi
  ...
  for tab in $LAYOUT; do
    ...
    if ! echo "$tab" | grep -E ",|/" >/dev/null; then
...
run_unit_tests() {
  ...
  for i in $(seq -w 1 99); do
    if test "$i" -gt $NUMBER_OF_TEST_CASES; then
      break
    fi
    ...
    if test "$FINAL_CMD" = "$(eval echo \${EXPECTED_CMD_"$i"})" &&
      test "$FINAL_TARGET" = "$(eval echo \${EXPECTED_TARGET_"$i"})"; then
  ...
  if test "$at_least_one_ko" = "true"; then
    return 255
  else
    return 0
  fi
