...
func (c *Organize) RunWithIds(u *umwelt.Umwelt, ids id_set.Set) (err error) {
	...
	if err = u.StoreObjekten().Zettel().ReadAllSchwanzenVerzeichnisse(wk, w); err != nil {
		err = errors.Wrap(err)
		return
	}
	...
	case organizeModeCommitDirectly:
		...
		if f, err = files.TempFileWithPattern("*." + u.Konfig().Compiled.DefaultOrganizeExt); err != nil {
			err = errors.Wrap(err)
			return
		}
